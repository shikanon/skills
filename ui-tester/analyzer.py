...
ANALYSIS_PROMPT = """你是一位资深的 UI/UX 设计评审专家。请对以下网页截图进行全面的视觉与体验分析。
...
8. **用户体验（UE）**：信息架构是否合理、操作流程是否顺畅、关键信息是否突出、用户引导是否清晰、是否存在认知负担或操作障碍
9. **视觉美观性**：整体视觉协调性、留白节奏感、图文搭配和谐度、品牌调性一致性、是否存在视觉杂乱或失衡
...
请以 JSON 格式输出：
{
  ...
  "user_experience": {
    "information_architecture": "信息架构是否合理，层级是否清晰",
    "flow_smoothness": "操作流程是否顺畅，步骤是否合理",
    "key_info_highlight": "关键信息是否突出，CTA是否明确",
    "user_guidance": "用户引导是否清晰，新手是否容易上手",
    "cognitive_load": "是否存在认知负担，信息是否过载",
    "issues": ["发现的具体UE问题"]
  },
  "visual_aesthetics": {
    "overall_harmony": "整体视觉协调性评分（1-10）及说明",
    "whitespace_rhythm": "留白节奏感评价",
    "image_text_balance": "图文搭配和谐度评价",
    "brand_consistency": "品牌调性一致性评价",
    "visual_clutter": "是否存在视觉杂乱或失衡",
    "strengths": ["视觉亮点1", "视觉亮点2"],
    "weaknesses": ["视觉不足1", "视觉不足2"]
  },
